...
class DaysRecordMachine extends RecordMachine
{
  ...
  //ユーザ名、ユーザid,緯度,経度を登録
  public function resisterUser($userName, $userId, $latitude, $longitude,$wardnumber=7)
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  public function setUserTimeValue($userName, $time, $value,$wardnumber=7)
  {
  ...
  public function addUserTimeValue($userName, $select,$wardnumber=7)
  {
  ...
  //二進数で指定した時間の値を1つ増やすに値をセット("Yamashita,0,any")
  public function addUseAppDays($userName,$wardnumber=7)
  {
    $str = "delivery_record_".$wardnumber;
    try{
  ...
  //二進数で指定した時間の値を1つ増やすに値をセット("Yamashita,0,any")
  public function addTotalDelivaryDays($userName,$wardnumber=7)
  {
    $str = "delivery_record_".$wardnumber;
    try{
  ...
  public function getUserPosition($userName,$wardnumber=7){
   ...
   public function getUsersValueArray($time,$wardnumber=7){
    try{
      if($time < 0 && $time > 25){
  ...
  public function getUserPositionArray($userName,$wardnumber=7){
...
class DeliveryRecordMachine extends RecordMachine
{
  ...
  //新商品の追加
  public function resisterDelivery($userName, $deliverynumber, $companyflag, $deliveryname,$scheduledday, $scheduletime=null,$wardnumber=7)
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  //
  public function setCompanyRequest($userName,$deliverynumber,$companyflag,$scheduledday,$scheduletime=null,$wardnumber=7){
    try{
  ...
  //
  public function setUserRequest($userName,$scheduleflag,$starttime,$endtime,$today,$wardnumber=7){
    try{
      $str = "delivery_".$wardnumber;
  ...
  public function serchDelivery($deliveryNumber, $companyflag,$wardnumber=7){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

      $stmt = $this->db->prepare("select* from delivery_7 where deliverynumber = :deliverynumber and companyflag = :companyflag");
  ...
  public function getUserDelivery($userName,$wardnumber=7){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

      $stmt = $this->db->prepare("select companyflag, deliverynumber, deliveryname, scheduledday, scheduletime from delivery_".$wardnumber." where userName = :userName");
      $stmt->execute([":userName"=>"YamashitaKeisuske"]);
