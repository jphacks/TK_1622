...
class UserRecordMachine extends RecordMachine
{
  ...
  //会員登録メソッド
  public function resisterUser($userName, $userId, $hashPassword, $latitude, $longitude, $postal=null, $prefecture=null, $ward=null, $address=null, $apartment=null,$phonenumber,$wardnumber=7)
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

      $stmt = $this->db->prepare("insert into user_record_jp (userName, userId, hashPassword, latitude, longitude, postal, prefecture, ward, address, apartment,phonenumber ,created) values (:userName, :userId, :hashPassword, :latitude, :longitude, :postal, :prefecture, :ward, :address, :apartment, :phonenumber,now())");
  ...
  public function getHashPassword($userName){
    try{
      ...
      $stmt = $this->db->prepare("select hashPassword from user_record_jp where userName like :username");
  ...
  public function serchUser($userName){
    try{
      ...
      $stmt = $this->db->prepare("select * from user_record_jp where userName = :username");
      $stmt->execute([":username"=>$userName]);
...
class DaysRecordMachine extends RecordMachine
{
  ...
  //ユーザ名、ユーザid,緯度,経度を登録
  public function resisterUser($userName, $userId, $latitude, $longitude,$wardnumber="jp")
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  public function setUserTimeValue($userName, $time, $value,$wardnumber="jp")
  {
  ...
  public function addUserTimeValue($userName, $select,$wardnumber="jp")
  {
  ...
  //二進数で指定した時間の値を1つ増やすに値をセット("Yamashita,0,any")
  public function addUseAppDays($userName,$wardnumber="jp")
  {
    $str = "delivery_record_".$wardnumber;
    try{
  ...
  //二進数で指定した時間の値を1つ増やすに値をセット("Yamashita,0,any")
  public function addTotalDelivaryDays($userName,$wardnumber="jp")
  {
    $str = "delivery_record_".$wardnumber;
    try{
  ...
  public function getUserPosition($userName,$wardnumber="jp"){
    try{
   ...
   public function getUsersValueArray($time,$wardnumber="jp"){
    try{
      if($time < 0 && $time > 25){
  ...
  public function getUserPositionArray($userName,$wardnumber="jp"){
...
class DeliveryRecordMachine extends RecordMachine
{
  ...
  //新商品の追加
  public function resisterDelivery($userName, $deliverynumber, $companyflag, $deliveryname,$scheduledday, $scheduletime=null,$wardnumber='jp')
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  //
  public function setCompanyRequest($userName,$deliverynumber,$companyflag,$scheduledday,$scheduletime=null,$wardnumber='jp'){
    try{
  ...
  //
  public function setUserRequest($userName,$scheduleflag,$starttime,$endtime,$today,$wardnumber='jp'){
    try{
      $str = "delivery_".$wardnumber;
  ...
  public function serchDelivery($deliveryNumber, $companyflag,$wardnumber='jp'){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $str = "delivery_".$wardnumber;
      $stmt = $this->db->prepare("select* from delivery_jp where deliverynumber = :deliverynumber and companyflag = :companyflag");
      $stmt->execute([":deliverynumber"=>$deliveryNumber, ":companyflag"=>$companyflag]);
      $results = count($stmt->fetchAll());
      // var_dump($results);
      return $results;
  ...
  public function getUserDelivery($userName,$wardnumber='jp'){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

      $stmt = $this->db->prepare("select companyflag, deliverynumber, deliveryname, scheduledday, scheduletime from delivery_".$wardnumber." where userName = :userName order by scheduledday,companyflag");
      $stmt->execute([":userName"=>$userName]);
  ...
  public function getTodayDelivery($userName,$wardnumber='jp'){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $today = date("Y-m-d");
      $stmt = $this->db->prepare("select companyflag, deliveryname, scheduletime from delivery_".$wardnumber." where userName = :userName and scheduledday = :today order by companyflag");
      $stmt->bindParam(":userName", $userName, \PDO::PARAM_STR);
      $stmt->bindParam(":today", $today, \PDO::PARAM_STR);
      $stmt->execute();
      $results = $stmt->fetchAll();
  ...
  public function getTommorowDelivery($userName,$wardnumber='jp'){
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $today = date("Y-m-d");
      $stmt = $this->db->prepare("select companyflag, deliveryname, scheduledday from delivery_".$wardnumber." where userName = :userName and scheduledday not in (:today) order by scheduledday,companyflag");
      $stmt->bindParam(":userName", $userName, \PDO::PARAM_STR);
      $stmt->bindParam(":today", $today, \PDO::PARAM_STR);
      $stmt->execute();
      $results = $stmt->fetchAll();
  ...
  //同一商品番号が存在するかチェック
  public function checkDeliveryNumber($deliveryNumber,$companyflag)
  {
    try{
      // $this->db = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD);
      // $this->db -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      if($this->serchDelivery($deliveryNumber,$companyflag)){
        return false;
      }else{
        return true;
      }
...
// $Yam->addUserTimeValue("YamashitaKeisuke",0b00001111000011110000);
...
// $Yam->addTotalDelivaryDays("YamashitaKeisuke");
// $Yam->addUseAppDays("YamashitaKeisuke");
// var_dump($Yam->getUsersValueArray(6));
